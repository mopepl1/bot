...
async def gen_cmd(client, message):
    try:
        ...
        try:
            ...
            # cc_parts = ccsdata.split(r'[\\/|]')


            cc_parts = re.split(r'[\\/|]', ccsdata)
...
Message: Maximum Generated Amount is 10K.</b>"""
        ...
        if amount == 10:
            resp = (
                ...
                f"- 𝐂𝐡𝐞𝐜𝐤𝐞𝐝 - <a href='tg://user?id={message.from_user.id}'> {message.from_user.first_name}</a>"
            )
...
- 𝐂𝐡𝐞𝐜𝐤𝐞𝐝 - <a href="tg://user?id={message.from_user.id}"> {message.from_user.first_name}</a>
"""
            await client.delete_messages(message.chat.id, delete.id)
