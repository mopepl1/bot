...
async def get_charge_resp(result, user_id, fullcc):
    try:
        ...
        if type(result) == str:
            ...
            or '"result":"success"' in result.text
            or '"status":"success"' in result.text
            or '"status":"successed"' in result.text
            or "Payment has been successfully processed" in result.text
            ...
            hits     = "YES"
            await forward_resp(fullcc, "NON SK CVV 1$ [/st]", response)
            ...
            or "Your card's security code is invalid" in result.text
        ):
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
            response = "CCN Live ❎"
            hits     = "YES"
        ...
        elif "No such PaymentMethod" in result.text:
            status   = "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌"
            ...
            or "Card Authentication needed" in result.text
            or "#wc-stripe-confirm-pi" in result.text
            or "Verifying strong customer authentication." in result.text
        ):
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
