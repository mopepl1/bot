...
async def get_charge_resp(result, user_id, fullcc):
    try:
        ...
        if type(result) == str:
            ...
            or "Thank you for your order," in result.text
            or "Thank You for your order," in result.text
            or "EXISTING_ACCOUNT_RESTRICTED" in result.text
            ...
            hits     = "YES"
            await forward_resp(fullcc, "Paypal Charge 1$", response)
            ...
            or "INVALID_SECURITY_CODE" in result.text
        ):
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
