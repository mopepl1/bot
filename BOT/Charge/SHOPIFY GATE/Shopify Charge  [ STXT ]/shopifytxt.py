...
async def chktxt2func(fullcc, user_id, bearer_token):
    result = await create_shopify_charge(fullcc, bearer_token)
...
@Client.on_message(filters.command("dontxt", [".", "/"]))
...
Upgrade to Premium using the /buy command to continue enjoying personal access.
        ...
        try:
            ...
            key = f"dontxt{message.from_user.id}_{random_text}"
            file_name = f"{key}.txt"
            hitsfile = f"HITS/{file_name}"
...
Usage: /dontxt [ in reply to txt file ]
"""
            await message.reply_text(resp, message.id)
            return
...
Message: YOu can't check more than 50 CCs at a time.
                """
