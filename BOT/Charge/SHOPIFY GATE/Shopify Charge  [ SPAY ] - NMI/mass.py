...
@Client.on_message(filters.command("mspay", [".", "/"]))
...
async def stripe_mass_auth_cmd(Client, message):
    try:
        ...
        ccs = getcc[1]
        resp = f"""
- 𝐆𝐚𝐭𝐞𝐰𝐚𝐲 - Shopify + Oceanpayment [18.89$]
...
- 𝐒𝐭𝐚𝐭𝐮𝐬 - Processing...⌛️
        ...
        text = f"""
<b>↯ MASS Shopify + Oceanpayment [18.89$] [/mspay]

Number Of CC Check : [{len(ccs)} / 15]
</b> \n
"""
        amt = 0
        start = time.perf_counter()
        proxies    = await get_proxy_format()
        session    = httpx.AsyncClient(timeout= 60, follow_redirects=True,proxies=proxies )
