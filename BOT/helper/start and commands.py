...
Angel Checker  Has plenty of Commands . We Have Auth Gates , Charge Gates , Tools And Other Things .
...
<b>Welcome aboard the Angel Checker! 🚀</b>
...
Click Each of Them Below to Know Them Better.</b>
    ...
    if CallbackQuery.data == "close":
       ...
       # await CallbackQuery.message.reply_text("Enjoy Dadu, @ANG0X4BOT")
...
Click on each one below to get to know them better. .</b>
    ...
    if CallbackQuery.data == "Auth2":
        CHARGE_TEXT = """
🔹 Stripe Auth Gates of Angel Checker
🔹 Status: ✅ Active
...
👤 Stripe Auth Options:
   ...
   1. Stripe Auth:
      ...
      ➜ txt: /authtxt cc|mm|yy|cvv ❌


   2. Stripe Auth 2:
      ➜ Single: /auth cc|mm|yy|cvv ✅
      ➜ Mass: /mauth cc|mm|yy|cvv ✅
      ➜ txt: /authtxt cc|mm|yy|cvv ❌

   3. Stripe Auth 3:
      ➜ Single: /cc cc|mm|yy|cvv ✅
      ➜ Mass: /mcc cc|mm|yy|cvv ✅
...
"""
    ...
    if CallbackQuery.data == "Adyen2":
        CHARGE_TEXT = """
🔹 Adyen Auth Gates of Angel Checker
🔹 Status: Inactive ❌
...
"""
    ...
    if CallbackQuery.data == "BRAINTREEVBV":
        CHARGE_TEXT = """
🔹 Braintree Gates of Angel Checker
🔹 Status: ✅ Active
...
"""
    ...
    if CallbackQuery.data == "BRAINTREEB3":
        CHARGE_TEXT = """
🔹 Braintree B3 of Angel Checker
🔹 Status: ✅ Active

🚀 Quick Commands Overview:

👤 Braintree B3 Auth Options:
   1. Braintree B3 Gate:
      ➜ Single: /b3 cc|mm|yy|cvv ✅
      ➜ Mass: /mb3 cc|mm|yy|cvv ✅

   2. Braintree B3 Gate:
      ➜ Single: /chk cc|mm|yy|cvv ✅
      ➜ Mass: /mchk cc|mm|yy|cvv ✅

   3. Custom Braintree B3 Gate:
      ➜ Single: /bra cc|mm|yy|cvv ✅
      ➜ Mass: /mbra cc|mm|yy|cvv ✅
...
Click on each one below to get to know them better. .</b>
    ...
    if CallbackQuery.data == "PAYPAL":
        CHARGE_TEXT = """
🔹 PayPal Charge Gates of Angel Checker
🔹 Status: ✅ Active

🚀 Quick Commands Overview:

👤 PayPal Charge Options:
   1. PayPal Charge 0.01$:
      ➜ Single: /pp cc|mm|yy|cvv [ON] ✅
      ➜ Mass: /mpp cc|mm|yy|cvv [ON] ✅

   2. PayPal Charge 1$:
      ➜ Single: /py cc|mm|yy|cvv [ON] ✅
...
"""
    ...
    if CallbackQuery.data == "SKBASED":
        CHARGE_TEXT = """
🔹 Stripe Charge Gates of Angel Checker
🔹 Status: ✅ Active
...
"""
    ...
    if CallbackQuery.data == "SITE":
        CHARGE_TEXT = """
🔹 Site Charge Gates of Angel Checker
🔹 Status: ✅ Active
...
👤 Site Charge Options:
   ...
   1. NONSK CVV CHARGE 1$:
      ➜ Single: /bb cc|mm|yy|cvv ✅
      ➜ Mass: /mbb cc|mm|yy|cvv ✅

   2. NONSK CVV CHARGE 3$:
...
"""
    ...
    if CallbackQuery.data == "BRAINTREE":
        CHARGE_TEXT = """
🔹 Braintree Charge Gates of Angel Checker
🔹 Status: ❌ Active
...
🔹 Shopify Charge Gates of Angel Checker
🔹 Status: ✅ Active
...
1. Shopify Normal 4$:
    ➜ Single: /sh cc|mm|yy|cvv『 FREE 💸 』 
    ➜ Mass: /msh cc|mm|yy|cvv 

2. Shopify Normal 32.72$: 
    ➜ Single: /sho cc|mm|yy|cvv 『 FREE 💸 』 
    ➜ Mass: /msho cc|mm|yy|cvv 

3. Shopify + PayFlow Pro 12.9:
    ➜ Single: /spf cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /mspf cc|mm|yy|cvv 

4. Shopify + Stripe 33.99$: 
    ➜ Single: /so cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /mso cc|mm|yy|cvv 

5. Shopify Normal 19.99$:
    ➜ Single: /sha cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /msha cc|mm|yy|cvv 

6. Shopify Normal 36.56$:
    ➜ Single: /shd cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /mshd cc|mm|yy|cvv 

7. Shopify + Oceanpayment 18.89$:
    ➜ Single: /spay cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /mspay cc|mm|yy|cvv 

8. Shopify + UseePay 65.0$:
    ➜ Single: /sup cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /msup cc|mm|yy|cvv 

9. Shopify + Paypal Pro 9.68$:
    ➜ Single: /spp cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /mspp cc|mm|yy|cvv 

10. Shopify + AuthorizeNet 13.93$:
    ➜ Single: /sau cc|mm|yy|cvv 『 FREE 💸 』
    ➜ Mass: /msau cc|mm|yy|cvv 
...
"""
    ...
    if CallbackQuery.data == "SCRAPPER":
        CHARGE_TEXT = """
🔹 Scrapper Tools Gates of Angel Checker
🔹 Status: ✅ Active
...
"""
    ...
    if CallbackQuery.data == "GENARATORTOOLS":
        CHARGE_TEXT = """
🔹 Generator Tools of Angel Checker
🔹 Status: ✅ Active
...
"""
    ...
    if CallbackQuery.data == "BINANDOTHERS":
        CHARGE_TEXT = """
🔹 Bin and Other Tools Of Angel Checker
🔹 Status: ✅ Active
...
👤 BIN Information:
   ...
   4. Find Bug Bin using /bugbin [in reply to text] ✅
...
Click on each one below to get to know them better.</b>
    ...
    if CallbackQuery.data == "INFO":
        CHARGE_TEXT = """
🔹 Helper Gates of Angel Checker
🔹 Status: ✅ Active

🚀 Quick Commands Overview:

👤 Account Management:
   1. Start Bot: /start@ANG0X4BOT
   2. Register: /register
   3. User ID: /id
   4. User Info: /info
