...
async def superchatdel(chat_id):
    super_chat.delete_one({"id": chat_id})



async def addsupergroup(chat_id):
    super_chat.insert_one({"id": chat_id, "status": "sp-approved"})


async def getsuperchatinfo(chat_id):
    return super_chat.find_one({"id": chat_id}, {"_id": 0})
...
async def deductcredit(user_id, message):
    user_id = str(user_id)
    ...
    if "∞" in get_user_info["plan"]:
        return
    elif chat_type in ["ChatType.GROUP", "ChatType.SUPERGROUP"] and supertgroup != "None":
        return
...
async def massdeductcredit(user_id,message, amt):
    user_id = str(user_id)
    chat_type = str(message.chat.type)
    chat_id   = str(message.chat.id)
    supertgroup     = await getsuperchatinfo(chat_id)
